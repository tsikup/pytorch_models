...
import torch
from pytorch_models.models.base_fair import BaseMILModel_GroupDRO
from pytorch_models.models.classification.clam import CLAM_SB


class CLAM_GroupDRO_PL(BaseMILModel_GroupDRO):
    def __init__(
        self,
        config,
        n_classes: int,
        ...
        super(CLAM_GroupDRO_PL, self).__init__(
            config,
            n_classes=n_classes,
            n_groups=n_groups,
    ...
    def forward(self, batch, is_predict=False):
        ...
        if not is_predict:
            # Loss (on logits)
            loss = self.dro_loss(logits, target.view(-1), sensitive_attr.view(-1))

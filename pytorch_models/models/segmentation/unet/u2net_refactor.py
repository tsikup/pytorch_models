...
Qin, X., Zhang, Z., Huang, C., Dehghan, M., Zaiane, O.R. and Jagersand, M., 2020. U2-Net: Going deeper with nested
U-structure for salient object detection. Pattern recognition, 106, p.107404.
"""
...
class U2NET(BaseModel):
    def __init__(self, config, cfgs, out_ch):
...
def U2NET_full(config):
    ...
    return U2NET(config, cfgs=full, out_ch=1)


def U2NET_lite(config):
    lite = {
        # cfgs for building RSUs and sides
        # {stage : [name, (height(L), in_ch, mid_ch, out_ch, dilated), side]}
    ...
    return U2NET(config, cfgs=lite, out_ch=1)

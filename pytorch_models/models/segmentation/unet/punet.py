...
Kohl, S., Romera-Paredes, B., Meyer, C., De Fauw, J., Ledsam, J.R., Maier-Hein, K., Eslami, S.M., Jimenez Rezende,
D. and Ronneberger, O., 2018. A probabilistic u-net for segmentation of ambiguous images. Advances in neural
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.layers.apply(
            lambda module: init_weights(net=module, init_type="kaiming", init_bias=True)
        )
            ...
            if initializers["w"] == "orthogonal":
                self.layers.apply(
                    lambda module: init_weights(
                        net=module, init_type="orthogonal", init_bias=True
                    )
                )
                self.last_layer.apply(
                    lambda module: init_weights(
                        net=module, init_type="orthogonal", init_bias=True
                    )
                )
                ...
                self.layers.apply(
                    lambda module: init_weights(
                        net=module, init_type="kaiming", init_bias=True
                    )
                )
                self.last_layer.apply(
                    lambda module: init_weights(
...
class ProbabilisticUnet(BaseModel):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            num_filters: is a list consisint of the amount of filters per layer
            latent_dim: dimension of the latent space
            no_convs_fcomb: no convs per block in the (convolutional) encoder of prior and posterior
        """

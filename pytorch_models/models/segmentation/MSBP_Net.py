...
Vuong, T.T., Song, B., Kim, K., Cho, Y.M. and Kwak, J.T., 2021. Multi-scale binary pattern encoding network for
cancer classification in pathology images. IEEE Journal of Biomedical and Health Informatics, 26(3), pp.1152-1163.
"""
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, infeat, outfeat, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(
            outfeat[0], outfeat[1], kernel_size=3, stride=stride, padding=1, bias=False
        )
...
class ResNet_MSBP(nn.Module):
    def __init__(self, exp_mode, freeze=False):
        super(ResNet_MSBP, self).__init__()
        self.exp_mode = exp_mode
        self.nr_classes = self.n_classes
    ...
    def _make_res_block(self, infeat, outfeat, nr_blocks, stride=1):
        ...
        if stride != 1 or infeat != outfeat[-1]:
            downsample = nn.Sequential(
                nn.Conv2d(
                    infeat, outfeat[-1], kernel_size=1, stride=stride, bias=False
                ),
    ...
    def forward(self, imgs):
        def scale_to(x, size):
            return nn.functional.interpolate(
                x, size=tuple(size), mode="bilinear", align_corners=True
            )
        ...
        if self.exp_mode == "ResNet":
            ...
            binary_code = torch.where(
                binary_code >= 0, torch.tensor(1).to("cuda"), torch.tensor(0).to("cuda")
            )

            binary_code[:, :, :, :, 1] = torch.mul(
                binary_code[:, :, :, :, 1], torch.tensor(2).to("cuda")
            )
            binary_code[:, :, :, :, 2] = torch.mul(
                binary_code[:, :, :, :, 2], torch.tensor(4).to("cuda")
            )
            binary_code[:, :, :, :, 3] = torch.mul(
                binary_code[:, :, :, :, 3], torch.tensor(8).to("cuda")
            )
            binary_code[:, :, :, :, 4] = torch.mul(
                binary_code[:, :, :, :, 4], torch.tensor(16).to("cuda")
            )
            ...
            if "zm" in self.exp_mode:
                ...
                scale_feat = [
                    self.gap(subtract[..., i]).view(scale_feat.size(0), -1)
                    for i in range(subtract.shape[-1])
                ]

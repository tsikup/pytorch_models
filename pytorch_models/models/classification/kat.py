...
class KernelAttention(nn.Module):
    ...
    def forward(self, x, kx, krd, clst, att_mask=None, l_debug_idx=0):
        ...
        t_q, t_k, t_v = map(
            lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), c_qkv
        )
        k_q, k_k, k_v = map(
            lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), k_kqv
        )
        c_q, _, _ = map(
            lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), c_kqv
        )
        ...
        if att_mask is not None:
            k_dots = k_dots.masked_fill(
                att_mask.permute(0, 1, 3, 2), torch.tensor(-1e9)
            )
...
class KATBlocks(nn.Module):
    def __init__(self, npk, dim, depth, heads, dim_head, mlp_dim, dropout=0.0):
        super().__init__()
...
class KATCL(nn.Module):
    ...
    def _update_moving_average(self):
        for online_params, target_params in zip(
            self.online_kat.parameters(), self.target_kat.parameters()
        ):
            target_params.data = (
